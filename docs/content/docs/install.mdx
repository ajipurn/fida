...
`fida off` removes Fida-managed agent integrations and setup metadata while preserving non-Fida config; `fida off <agent>` disables a single agent. Delete the binary yourself when you are done — Fida never removes its own running executable.
