...
Run one command to wire supported agents, verify redaction, and scan the repository:
...
```bash
fida init --yes
fida init --agents opencode,cursor
fida init --all
fida init --project
```

Global setup is the default. Project setup writes project-scoped config where the target agent supports it.
...
The three layers are not equally strong on every agent, and Fida is explicit about it. `fida init`, `fida status`, and `fida doctor` use the same protection model:
...
Run `fida status` for the day-to-day report and `fida doctor` when diagnosing gaps. For best-effort agents, add OS-level controls when you need containment that does not depend on the agent following steering. See the [security model](/docs/concepts/security-model) for how the layers stack.

### Managed blocks

Files shared with you, such as `CLAUDE.md`, `AGENTS.md`, `~/.gemini/GEMINI.md`, and Windsurf global rules, are edited through managed blocks. Your own content is preserved during init and uninstall.
...
Uninstall removes Fida-managed blocks and setup metadata. It does not delete unrelated agent configuration.
