...
Fida is not a general policy engine, agent approval layer, or development firewall. It does not decide which normal commands an agent may run, which files it may edit, or whether a package install needs approval.
...
`fida init` wires selected agent integrations, runs a synthetic-secret self-test, and scans the repository. `fida status` reports protection as `enforced`, `best_effort`, `incomplete`, or `inactive`. `fida scan` never emits a secret value, length, or fragment.
